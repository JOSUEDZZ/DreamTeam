...
class PlatillosFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        CoroutineScope(Job()).launch {
            val strPlatillos =
                WebServiceREST.get("http://192.168.0.23/restaurant/public/platillos")
            ...
            val adaptador = PlatillosAdapter(listaPlatillos,
                {
                    (requireActivity() as MainActivity).dispatchTakePictureIntent()
                //(requireActivity() as MainActivity).cambarAlFractoLogin()
                }, requireActivity()
            )
